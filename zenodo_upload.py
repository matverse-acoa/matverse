...
def create_deposition():
    metadata = {
        'metadata': {
            'title': 'symbiOS: Minimal Sovereign Informational Kernel',
            'upload_type': 'software',
            'description': 'Minimal Sovereign Informational Kernel with MNB and COER architecture.',
...
if __name__ == "__main__":
    if not ACCESS_TOKEN:
        print("Zenodo API token not found in environment variables.")
    else:
        deposition = create_deposition()
        if deposition:
            dep_id = deposition['id']
            bucket_url = deposition['links']['bucket']
            upload_file(dep_id, bucket_url, '/home/ubuntu/symbios_project/symbios-core.pdf')
            upload_file(dep_id, bucket_url, '/home/ubuntu/symbios_project/symbios-core-v1.tar.gz')
            published = publish_deposition(dep_id)
            if published:
                print(f"Published successfully. DOI: {published['doi']}")
                with open('/home/ubuntu/symbios_project/zenodo_doi.txt', 'w') as f:
