...
def create_deposition():
    metadata = {
        'metadata': {
            'title': 'symbiOS v1.1: Institutional Closure Update',
            'upload_type': 'software',
            'description': 'Updated kernel with Institutional Closure metrics (Omega-Closure) and Genesis Integrity.',
...
if __name__ == "__main__":
    if not ACCESS_TOKEN:
        ...
        if deposition:
            ...
            upload_file(dep_id, bucket_url, '/home/ubuntu/symbios_project/symbios-core-v2.pdf')
            upload_file(dep_id, bucket_url, '/home/ubuntu/symbios_project/symbios-core-v2.tar.gz')
            published = publish_deposition(dep_id)
            if published:
                print(f"Published successfully. DOI: {published['doi']}")
                with open('/home/ubuntu/symbios_project/zenodo_doi_v2.txt', 'w') as f:
